...
def get_historical_weather_context(config: MonitorConfig, *, now: datetime) -> HistoricalWeatherContext:
    longitude, latitude = _centroid(config.polygon)
    ...
    try:
        ...
        daily = payload["daily"]
        precipitation = _numbers(daily.get("precipitation_sum", []))
        temperature_max = _numbers(daily.get("temperature_2m_max", []))
        wind_gusts_kmh = _numbers(daily.get("wind_gusts_10m_max", []))
        evapotranspiration = _numbers(daily.get("et0_fao_evapotranspiration", []))
...
def get_elevation_context(config: MonitorConfig) -> ElevationContext:
    center_lon, center_lat = _centroid(config.polygon)
    ...
    try:
        ...
        elevations = _numbers(payload.get("elevation", []))
    except Exception as exc:
...
def _numbers(values: list[float | int | None]) -> list[float]:
